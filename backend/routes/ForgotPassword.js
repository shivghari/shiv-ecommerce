...
router.post('/getToken', (req,res)=>{
    ...
    User.find({ username : req.body.username }).then((response)=>{
        ...
        if(response[0].username === req.body.username){
            ...
            res.status(200).json({ forgotPassToken : token, email : response[0].email, userID : response[0]._id })
        }else{
...
router.post('/changePassword',verifyToken, (req, res)=>{
    ...
    User.updateOne({ _id : req.body.userID }, {$set : {
        ...
        res.status(200).json({ message : "Password Change Successfully", response })
    }).catch((err)=>{
        res.status(200).json({ message : "SomeThing Went Wrong in Updating Password" })
    })
