...
router.post("/addRating", (req,res)=>{
    req.body.ratingData.map((item)=>{
        Product.find({ _id : item.productID }, {rating : 1, ratevotecount : 1}).then((response)=>{
            console.log('default Ratoing',response[0].rating )
            console.log('rate vote count', response[0].ratevotecount)
            console.log('new Rating', item.rating)
            var newRating
            ...
            Product.updateOne({  _id : item.productID }, {$set : {
                ...
                console.log(err,'err')
                res.status(300).json({ message : "Something Went Wrong in Product Rating" })
            })
        }).catch((err)=>{
            console.log(err)
        })
